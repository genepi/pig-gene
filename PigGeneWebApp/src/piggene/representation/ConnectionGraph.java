...
public class ConnectionGraph {
	...
	public static List<Connection> createConnectionList(final Workflow wf) throws JSONException, ConnectionGraphException {
		...
		for (final String parameterName : parameterMapping.getMatchingInAndOutputParameterValuesFromAllWorkflows()) {
			if (contentCheck(parameterName)) {
				...
				if (correspondingOutputParameterValues.size() > 1) {
					throw new ConnectionGraphException("Only one corresponding output parameter value allowed!");
				}
